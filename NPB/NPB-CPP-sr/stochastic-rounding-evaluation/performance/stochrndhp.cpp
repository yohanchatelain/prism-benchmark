...
double sr_mpfr(mpfr_t x, mpfr_t w1, mpfr_t w2) {
  ...
  int exponent = get_exponent<double, std::int64_t>(xd);
  mpfr_mul_2exp(w1, w1, 52 - exponent,
                MPFR_RNDN); // hardcoded for double precision
...
double sr_op_mpfr_noalloc(const double a, const double b,
                          int (*mpfr_op)(mpfr_ptr, mpfr_srcptr, mpfr_srcptr,
                                         mpfr_rnd_t),
...
double sr_op_mpfr_noalloc_single_arg(const double a,
                                     int (*mpfr_op)(mpfr_ptr, mpfr_srcptr,
                                                    mpfr_rnd_t),
                                     mpfr_t w1, mpfr_t w2, mpfr_t w3) {
  mpfr_set_d(w1, a, MPFR_RNDN);
